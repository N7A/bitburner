...
export async function main(ns: NS) {
    ...
    // si déjà actif
    if (ExecutionsRepository.getAll(ns).some(x => x.type === ProcessRequestType.SETUP_HACK && x.target === input.hostnameTarget)) {
        // on ne fait rien
        return;
    }

    ExecutionsRepository.add(ns, {type: ProcessRequestType.SETUP_HACK, target: input.hostnameTarget, weight: 1});
}
